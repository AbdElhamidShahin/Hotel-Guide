import 'package:flutter/material.dart';

class AppColors {
  static const Color gray = Color(0xCCDDDDDD);
  static const Color gray2 = Color(0xFF939393);
  static const Color gray3 = Color(0xFF4A4A4A);
  static const Color grayLight = Color(0x2BF3F3F3);
  static const Color grayLight2 = Color(0xFF2F2F2);
  static const Color white = Color(0xFFFFFFFF);
  static const Color blueSoft = Color(0xFFCFDBEC);
  static const Color warningColor = Color(0xFFC8C8F4);

  static const Color yellowSoft = Color(0xFFFFBD54);
  static const Color yellowGold = Color(0xFFFFC15F);
  static const Color orangeLight = Color(0xFFD98E4E);
  static const Color orangeGold = Color(0xFFFFBD54);
  static const Color orangeDark = Color(0xFFB25916);

  static const Color brownDark = Color(0xFF181201);
  static const Color black = Color(0xFF000000);
  static const Color black2 = Color(0xFF343434);
  static const Color black3 = Color(0xFF292929);
  static const Color black4 = Color(0xFF222222);
  static const Color black45 = Color(0x80222222);
  static const Color black5 = Color(0xFF1F1F1F);
  static const Color black6 = Color(0xFF171725);
  static const Color black7 = Color(0xFF101010);
  static const Color black8 = Color(0xFF1F1F1F);
}
